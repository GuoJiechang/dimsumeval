...
def mweval_sent(sent, ggroups, pgroups, gmwetypes, pmwetypes, stats, indata=None):
    # print(sent)
    # sent_zipped_list = list(zip(*sent))
    # print(sent_zipped_list)
    
    # print(sent)
    # #verify the taggings are valid
    # for k,kind in [(1,'gold'),(2,'pred')]:
    #     tags = sent_zipped_list[k]
    #     print(tags)
    ...
    #print(type(sent))
    list_sent = list(sent)
    for i,(tkn,goldTag,predTag) in enumerate(list_sent):
        #print(goldTag + " " + predTag)
        if goldTag!=predTag:
            stats['incorrect'] += 1
        else:
            stats['correct'] += 1
...
if __name__=='__main__':
    ...
    for sentId,gdata in readsents(fileinput.input(goldFP)):
        ...
        for predF,stats,gmwetypes,pmwetypes,sststats,conf in zip(predFs,statsCs,gmwetypesCs,pmwetypesCs,sststatsCs,confCs):
            ...
            try:
                #print(zip(words,gtags_mwe,ptags_mwe))
    ...
    if len(predFs)==1:
        ...
        for d,d2 in (('n.','v.'),('v.','n.')):
            ...
            #for lbl,n in sorted(gconfsC.most_common(),  key=lambda l: not l.startswith(d)):
